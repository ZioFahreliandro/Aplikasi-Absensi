...
async function loadRecapData() {
  ...
  try {
    ...
    currentAttendanceLogs.forEach(log => {
      ...
      const gpsDisplay = log.coords && log.coords.lat !== null
        ? `<span title="Lat: ${log.coords.lat.toFixed(6)}, Lng: ${log.coords.lng.toFixed(6)}">
      ...
      tr.innerHTML = `
        ...
        <td>${typeBadge}</td>
        <td>
          <img src="${log.selfieUrl}" class="table-selfie-thumb" alt="Selfie ${log.employeeName}">
        </td>
        <td>${gpsDisplay}</td>
...
function openModal(log) {
  ...
  img.src = log.selfieUrl;
  
  const formattedCoords = log.coords && log.coords.lat !== null 
  ...
  details.innerHTML = `
    <div>Karyawan: <span>${log.employeeName}</span></div>
    ...
    <div>IP Koneksi: <span>${log.ip || 'Localhost'}</span></div>
...
function exportRecapCSV() {
  ...
  currentAttendanceLogs.forEach(log => {
    ...
    const ip = log.ip || '-';
    const lat = log.coords && log.coords.lat !== null ? log.coords.lat : '-';
    const lng = log.coords && log.coords.lng !== null ? log.coords.lng : '-';
...
async function loadSettingsData() {
  try {
    ...
    document.getElementById('set-office-name').value = settings.officeName;
    document.getElementById('set-enable-gps').checked = settings.enableGps;
    document.getElementById('set-enable-ip').checked = settings.enableIp;
    document.getElementById('set-office-lat').value = settings.officeLat;
    document.getElementById('set-office-lng').value = settings.officeLng;
    document.getElementById('set-office-radius').value = settings.officeRadius;
    document.getElementById('set-office-ip').value = settings.officeIp;
