...
async function fetchEmployees() {
  try {
    ...
    selectEmployee.innerHTML = '<option value="" disabled selected>Pilih Karyawan...</option>';
    ...
    showToast("Gagal mengambil data karyawan dari server", "error");
  }
}
...
function updateValidationStatus() {
  if (!officeSettings.officeName) return; // settings not loaded yet
  
  let gpsValid = true;
  let ipValid = true;
  
  // 1. Geolocation Check
  if (officeSettings.enableGps) {
    if (userCoords.lat === null || userCoords.lng === null) {
      ...
      if (distance <= officeSettings.officeRadius) {
        badgeGps.className = "indicator-badge success";
        badgeGps.innerHTML = `<span class="pulse-dot"></span> GPS: Di Dalam Area`;
        txtGpsDetails.innerHTML = `<i data-lucide="map-pin"></i> Jarak ke kantor: <strong>${roundedDist}m</strong> (Batas: ${officeSettings.officeRadius}m)`;
      } else {
        gpsValid = false;
        badgeGps.className = "indicator-badge danger";
        badgeGps.innerHTML = `<span class="pulse-dot"></span> GPS: Di Luar Jangkauan`;
        txtGpsDetails.innerHTML = `<i data-lucide="map-pin"></i> Jarak Anda: <strong>${roundedDist}m</strong> (Batas: ${officeSettings.officeRadius}m)`;
      }
    }
  } else {
  ...
  if (officeSettings.enableIp) {
    if (!currentIp) {
    ...
    } else if (currentIp === officeSettings.officeIp || officeSettings.officeIp === '127.0.0.1') {
      ...
      badgeIp.className = "indicator-badge danger";
      badgeIp.innerHTML = `<span class="pulse-dot"></span> IP Jaringan: Tidak Valid`;
      txtIpDetails.innerHTML = `<i data-lucide="globe"></i> IP Anda: ${currentIp} (Harus IP Kantor: ${officeSettings.officeIp})`;
    }
  } else {
...
async function submitAttendance(type) {
  ...
  // Basic validation
  if (!employeeId) {
    showToast("Silakan pilih nama karyawan terlebih dahulu!", "error");
    return;
  }
  
  if (!pin || pin.length !== 4) {
    showToast("PIN wajib diisi dengan 4 angka!", "error");
    return;
  }
  ...
  try {
    const payload = {
      ...
      selfie: selfieBase64,
      coords: userCoords,
      pin // Sent for server-side verification
    };
...
window.fetchEmployees = fetchEmployees; // Expose to allow admin list updates to sync kiosk
